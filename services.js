define(function (require, exports, module) {
	...
	module.exports = function(app){
		...
		.factory('$rest', ["$rootScope", "$window", "$http", function($rootScope, $window, $http) {
			...
			return {
				qry_dists: function(cb){get("/api/dists", cb);},

				get_site: function(cb){get("/api/site", cb);},
				get_site_by_id: function(id,cb){get("/api/site/"+id, cb);},
				create_site_by_id: function(id,cb){get("/api/site/create/"+id, cb);},
				qry_sites_by_basesite: function(cb){get("/api/sites/basesite", cb);},
				qry_sites_by_owner: function(cb){get("/api/sites/owner", cb);},
				set_site: function(obj, cb){post("/api/site", obj, cb);},


				qry_themes: function(cb){get("/api/themes", cb);},
				qry_theme_configs: function(cb){get("/api/theme/configs", cb);},
				get_theme_config: function(theme, cb){get("/api/theme/config/" + theme, cb);},
				get_theme:function(cb){get("/api/theme", cb);},
				set_theme:function(params, cb){post("/api/theme", params, cb);},

				qry_blocks: function(cb){get("/api/blocks", cb);},
				del_block_by_id: function(id, cb){get("/api/block/del/" + id, cb);},
				set_block:function(params, cb){post("/api/block", params, cb);},

				qry_plugins_group: function(cb){get("/api/plugins/group", cb);},
				qry_plugins_by_group: function(group, cb){get("/api/plugins/"+group, cb);},

				qry_carousels:function(cb){get("/api/carousels", cb);},
				qry_carousels_by_key:function(key, cb){get("/api/carousels/key/"+key, cb);},
				del_carousel_by_id:function(id, cb){get("/api/carousel/del/"+id, cb);},
				set_carousel:function(params, cb){post("/api/carousel", params, cb);},

				qry_pages: function(cb){get("/api/pages", cb);},
				del_page_by_id: function(id, cb){get("/api/page/del/" + id, cb);},
				set_page: function(params, cb){post("/api/page", params, cb);},
				...
				set_user_profile: function(obj, cb){ post("/api/user/set_profile", obj, cb);},
				...
				get_cat_by_id: function(id, cb){ get("/api/cat/"+id, cb); },
				qry_cats: function(siteid, cb){if(_.isFunction(siteid)){ get("/api/cats", siteid);}else{get("/api/cats/siteid/" + siteid, cb);}},
				qry_cats_by_cattype: function(cattype, siteid, cb){if(_.isFunction(siteid)){ get("/api/cats/cattype/" + cattype, siteid);}else{get("/api/cats/cattype/" + cattype + "/siteid/" + siteid, cb);}},
				...
				qry_videos: function(siteid, cb) {if(_.isFunction(siteid)){ get("/api/videos", siteid);}else{get("/api/videos/siteid/" + siteid, cb);}},
				qry_videos_by_catid: function(catid, siteid, cb) {if(_.isFunction(siteid)){ get("/api/videos/catid/" + catid, siteid);}else{get("/api/videos/catid/" + catid + "/siteid/" + siteid, cb);}},
				qry_videos_by_keyword: function(keyword, siteid, cb) {if(_.isFunction(siteid)){ get("/api/videos/keyword/" + keyword, siteid);}else{get("/api/videos/keyword/" + keyword + "/siteid/" + siteid, cb);}},
				del_video_by_id: function(id, cb) { get("/api/video/del/"+id, cb);},
				set_video: function(params, cb){ post("/api/video", params, cb);},

				get_article_by_id: function(id, cb){ get("/api/article/"+id, cb); },
				qry_articles: function(siteid, cb) {if(_.isFunction(siteid)){ get("/api/articles", siteid);}else{get("/api/articles/siteid/" + siteid, cb);}},
				qry_articles_by_catid: function(catid, siteid, cb) {if(_.isFunction(siteid)){ get("/api/articles/catid/" + catid, siteid);}else{get("/api/articles/catid/" + catid + "/siteid/" + siteid, cb);}},
				qry_articles_by_keyword: function(keyword, siteid, cb) {if(_.isFunction(siteid)){ get("/api/articles/keyword/" + keyword, siteid);}else{get("/api/articles/keyword/" + keyword + "/siteid/" + siteid, cb);}},
				del_article_by_id: function(id, cb) { get("/api/article/del/"+id, cb);},
				set_article: function(params, cb){ post("/api/article", params, cb);},

				get_template_by_id: function(id, cb){ get("/api/template/"+id, cb); },
				qry_templates: function(cb){ get("/api/templates", cb);},
				del_template_by_id: function(id, cb) { get("/api/template/del/"+id, cb);},
				set_template: function(params, cb){post("/api/template", params, cb);},
				...
				qry_members_by_position:function(position, orgid, cb){if(_.isFunction(orgid)){ get("/api/members/position/" + position, orgid);}else{get("/api/member/position/" + position + "/orgid/" + orgid, cb);}},
				qry_members_by_role:function(role, orgid, cb){if(_.isFunction(orgid)){ get("/api/members/role/" + role, orgid);}else{get("/api/member/role/" + role + "/orgid/" + orgid, cb);}},
				qry_members:function(orgid, cb){if(_.isFunction(orgid)){ get("/api/members", orgid);}else{get("/api/members/orgid/" + orgid, cb);}},
				del_member_by_id:function(id, cb){get("/api/member/del/" + id, cb);},
				set_member:function(obj, cb){post("/api/member", obj, cb);},


				get_dists: function(cb){ get("/api/taobao/dists", cb);},
				get_logistics_cars: function(cb){ get("/api/logistics/cars", cb);},


				get_waybill_by_id: function(id, cb){ get("/api/waybill/" + id, cb);},
				get_waybill_link_by_id: function(id, cb){ get("/api/waybill/" + id + "/link", cb);},
				qry_waybills_by_opratorid: function(opratorid, cb){ get("/api/waybill/opratorid/" + opratorid, cb);},
				del_waybill_by_id: function(id, cb){ get("/api/waybill/del/" + id, cb);},
				set_waybill: function(params, cb){ post("/api/waybill/", params, cb);},

				get_addr_by_id: function(id, cb){ get("/api/addr/" + id, cb);},
				qry_addrs_by_custid: function(custid, cb){ get("/api/addr/custid/" + custid, cb);},
				del_addr_by_id: function(id, cb){ get("/api/addr/del/"+id, cb);},
				set_addr:function(params, cb){ post("/api/addr", params, cb);},

				qry_manufest_by_refid: function(refid, cb){ get("/api/manufest/refid/" + refid, cb);},
				del_manufest_by_refid: function(refid, cb){ get("/api/manufest/del/" + refid, cb);},
				del_item_by_id: function(id, cb){ get("/api/manufest/delitem/"+id, cb);},
				set_item:function(params, cb){ post("/api/manufest/item", params, cb);},

				get_order_by_id: function(id, cb){ get("/api/order/" + id, cb);},
				get_order_by_refid: function(refid, cb){ post("/api/order/refid/" + refid, order, cb);},
				qry_orders_by_buyerid: function(buyerid, cb){ get("/api/orders/buyerid/"+buyerid, cb);},
				qry_orders_by_salerid: function(salerid, cb){ get("/api/orders/salerid/" + salerid, cb);},
				qry_orders_by_ref: function(ref, cb){ get("/api/orders/ref/" + ref, cb);},
				del_order_by_id: function(id, cb){ get("/api/order/del/" + id, cb);},
				set_order: function(params, cb){ post("/api/order", params, cb);},

				get_cust_by_id: function(id, cb){ get("/api/cust/"+ id , cb);},
				get_cust_by_refid: function(refid, cb){ get("/api/cust/refid/" + refid,  cb);},
				qry_custs: function(cb){ get("/api/custs", cb);},
				qry_custs_by_pre: function(pre, cb){ get("/api/custs/pre/"+pre, cb);},
				qry_custs_by_ref: function(ref, cb){ get("/api/custs/ref/" + ref, cb);},
				del_cust_by_id: function(id, cb){ get("/api/cust/del/"+id, cb);},
				set_cust: function(params, cb){ post("/api/cust/", params, cb);},
			};
		}])		
