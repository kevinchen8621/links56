...
require_once SDK_PATH . './CosUtil.class.php';
...
class Cos {
    ...
    public function __construct() {
        $host = COS_HOST;
        $accessId = 11000643;
        $accessKey = "w6BFkfTQBogdwUKgzmUvz86fcDHqpUgL";
        $secretId = "AKIDspGFHghwxuBEld69v3u4yLA9tO8s3bNU";
        $this->host = $host;
    ...
    public function upload_x($arr){
        ...
        $result =array();
        foreach ($arr as $k=>$v) {
            $obj_name = time() .'_'.$i. '.' . $this->get_file_type($k);
            $main = $v;
            $ret = $this->upload_file_by_content($bucket, $path, $obj_name, $main);
            $url = "http://" . COS_DOWNLOAD_HOST . "/" . $bucketId . "/" . $bucket . $path . "/" . $obj_name;
            $i++;
     ...
     * @return array 结果数组
     */
    public function upload_file_content_with_compress($compressBucketId, $compressFilePath, $fileContent, $opt = array()) {
        $ret = array(
            'code' => 0,
            'msg' => 'ok',
        );
