...
class sender_controller extends cbase{
    //getname
    function getName(){return 'sender';}
    ...
    function change_action(){
        ...
        //数据库操作
        $rc = $this -> shippersql -> update_shipper_contact($arr,$shipper_staff['shipperid'],$userinfo['id']);
        //输出
        if(empty($rc)){
            $this-> xbase -> failed(99,'数据库异常，写入失败');
        }
    ...
    function del_action(){
        ...
        //输出
        if(empty($rc)){
            $this-> xbase -> failed(4,'常用发货人Id不存在');
        }
        $this -> shippersql -> memcache_save();
        $this -> xbase -> success();
    }
    ...
    function getlist_action(){
        ...
        $rc = $this -> shippersql -> get_shipper_contact($shipper_staff['shipperid'],1,$pg-1,$size);
        //输出
        if(empty($rc)){
