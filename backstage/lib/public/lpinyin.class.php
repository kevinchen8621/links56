...
class lpinyin { // 用户信息
    ...
    function getInitial($input, $_Code = 'UTF-8') { // 返回首字母
        if ($_Code == 'UTF-8') {
            $str = $this->_U2_Utf8_Gb($input);
        } else {
            $str = $input;
        }
        $all = str_split($str, 2);
        $rc = '';
        foreach ($all as $a) {
            $rc .= $this->_getInitial($a);
        }
        return $rc;
    }

    function getPinyin($_String, $_Code = 'UTF-8') { // gb2312
